...
test('renders active cards and editor across desktop and mobile', async ({ page, request }) => {
  ...
  const activeCard = page.locator('.event-card').filter({ hasText: eventTitle });
  await expect(activeCard.locator('.active-focus-node')).toHaveCount(1);
  ...
  expect(await currentFocusLayer.evaluate((element) => getComputedStyle(element).animationDuration)).toBe('0.28s');
